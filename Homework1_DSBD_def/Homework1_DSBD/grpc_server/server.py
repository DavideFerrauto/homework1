...
''' 
# Creazione di un dizionario con tupla come chiave
dizionario = {
    ('user_id_1', 'request_id_1'): 'response_1',
    ('user_id_1', 'request_id_2'): 'response_2'
}
'''
cache = {}
...
def process_with_cache(context, cache_lock, cache, process_function):
    ...
    # Memorizza la risposta nella cache inserendo come key la tupla (userid, requestid)
    with cache_lock:
    ...
    def GetLatestValue(self, request, context):
        ...
        def process_value():
            ...
            try:
                ...
                self.cur.execute("SELECT open, date FROM utenti JOIN sessioni_utenti ON utenti.id = sessioni_utenti.id_utente JOIN dati ON sessioni_utenti.id_dato = dati.id WHERE utenti.email = %s ORDER BY date DESC LIMIT 1;", (request.email,))
                result = self.cur.fetchone()
                if result:
                    valore, date = result
                    return pb2.StockValueResponse(success=True, value=valore, date=str(date))
                else:
    ...
    def GetAverageValue(self, request, context):
        ...
        def process_avg():
            ...
            try:
                ...
                self.cur.execute( "SELECT AVG(open) FROM (SELECT open FROM utenti JOIN sessioni_utenti ON utenti.id = sessioni_utenti.id_utente JOIN dati ON sessioni_utenti.id_dato = dati.id WHERE utenti.email = %s ORDER BY dati.date DESC LIMIT %s) AS subquery", (request.email, request.count))
                result = self.cur.fetchone()
                return pb2.StockValueResponse(success=True, value=result[0])
